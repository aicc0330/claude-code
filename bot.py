...
def print_qr_terminal(data: str, label: str = "") -> None:
...
def make_qr_image_bytes(data: str) -> bytes:
    """Return a PNG QR code image as bytes."""
...
async def cmd_qr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send a QR code image for this bot's Telegram link."""
    assert update.message and context.bot
    me = await context.bot.get_me()
    url = f"https://t.me/{me.username}"
    png = make_qr_image_bytes(url)
    await update.message.reply_photo(photo=png, caption=f"Scan to open: {url}")
...
async def main() -> None:
    ...
    tg_app.add_handler(CommandHandler("qr", cmd_qr))
    ...
    # Print QR code for the bot's Telegram link so the admin can scan it
    async with tg_app.bot:
        me = await tg_app.bot.get_me()
    bot_url = f"https://t.me/{me.username}"
    print_qr_terminal(bot_url, label="Scan to connect via Telegram:")
